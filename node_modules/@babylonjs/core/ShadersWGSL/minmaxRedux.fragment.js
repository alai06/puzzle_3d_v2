...
#if defined(INITIAL)
var sourceTexture: texture_2d<f32>;uniform texSize: vec2f;@fragment
fn main(input: FragmentInputs)->FragmentOutputs {let coord=vec2i(fragmentInputs.vUV*(uniforms.texSize-1.0));let f1=textureLoad(sourceTexture,coord,0).r;let f2=textureLoad(sourceTexture,coord+vec2i(1,0),0).r;let f3=textureLoad(sourceTexture,coord+vec2i(1,1),0).r;let f4=textureLoad(sourceTexture,coord+vec2i(0,1),0).r;let minz=min(min(min(f1,f2),f3),f4);
#ifdef DEPTH_REDUX
...
uniform texSize: vec2f;@fragment
fn main(input: FragmentInputs)->FragmentOutputs {let coord=vec2i(fragmentInputs.vUV*vec2(uniforms.texSize-1));let iTexSize=vec2i(uniforms.texSize);let f1=textureLoad(textureSampler,coord % iTexSize,0).rg;let f2=textureLoad(textureSampler,(coord+vec2i(1,0)) % iTexSize,0).rg;let f3=textureLoad(textureSampler,(coord+vec2i(1,1)) % iTexSize,0).rg;let f4=textureLoad(textureSampler,(coord+vec2i(0,1)) % iTexSize,0).rg;let minz=min(f1.x,f2.x);let maxz=max(f1.y,f2.y);fragmentOutputs.color=vec4(minz,maxz,0.,0.);}
#elif defined(LAST)
@fragment
